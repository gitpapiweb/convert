...
import {CurvedArrowDownIcon, KeyIcon} from './components/icons';
...
const App: React.FC = () => {
  ...
  const handleManageApiKey = useCallback(async () => {
    if (window.aistudio) {
      await window.aistudio.openSelectKey();
      // If user was in an error state, give them a fresh start
      // after potentially fixing their key.
      if (appState === AppState.ERROR) {
        handleNewVideo();
      }
    }
  }, [appState, handleNewVideo]);
    ...
    <div className="h-screen bg-black text-gray-200 flex flex-col font-sans overflow-hidden">
      ...
      <header className="py-6 flex justify-center items-center px-8 relative z-10">
        ...
        <button
          ...
          className="absolute right-8 top-1/2 -translate-y-1/2 flex items-center gap-2 px-4 py-2 bg-gray-700/50 hover:bg-gray-700 text-gray-300 hover:text-white font-medium rounded-lg transition-colors text-sm">
